...
The JSON-RPC server listens on port **`${{ values.rpcPort }}`** with user
`${{ values.rpcUser }}`. Requests are HTTP POSTs with basic auth.
...
`${{ values.componentId }}` → its system → its API.
