...
`${{ values.componentId }}` node. Follow it top to bottom for a first deploy, or
...
- Docker image `${{ values.imageRepo }}` reachable (built via CI or `build.sh`).
...
```bash
systemctl status ${{ values.componentId }}
docker ps | grep ${{ values.componentId }}
# RPC reachable?
curl -s --user ${{ values.rpcUser }}:<pass> \
  --data-binary '{"jsonrpc":"1.0","id":"1","method":"getblockcount","params":[]}' \
  -H 'content-type: text/plain;' http://127.0.0.1:${{ values.rpcPort }}/
```

## State & data

- Node state lives in the docker volume **`${{ values.componentId }}-data`**.
...
2. CI rebuilds `${{ values.imageRepo }}:<sha>`.
3. Update the systemd `Image=` line to the new tag, or use a mutable `latest`
   tag pointed at the newest build.
4. `sudo systemctl daemon-reload && sudo systemctl restart ${{ values.componentId }}`
