...
This page describes how the `${{ values.componentId }}` node is put together and
why. It's the mental model an operator (or platform engineer) needs before
working on the node.

## System context

The node is one component inside the **`${{ values.system }}`** system in the
...
```mermaid
graph LR
    subgraph System: ${{ values.system }}
        N[${{ values.componentId }}]
    end
    N -- "P2P (${{ values.p2pPort }})" --> PEERS[CivicNet peers]
    N -- "JSON-RPC (${{ values.rpcPort }})" --> CLI[${{ values.rpcUser }} / monitoring]
...
| `Dockerfile` | Builds the node binaries (`${{ values.imageRepo }}`) |
...
```text
systemd unit
  └─ docker run ${{ values.imageRepo }}:$(git sha)
       └─ civicnetd
