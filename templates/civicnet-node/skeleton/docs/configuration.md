...
|---------|-----------------|---------|
| `rpcuser` | `${{ values.rpcUser }}` | Username for JSON-RPC auth |
| `rpcpassword` | *(secret)* | Password for JSON-RPC auth — keep in Infisical |
| `rpcport` | `${{ values.rpcPort }}` | TCP port for JSON-RPC |
| `port` | `${{ values.p2pPort }}` | TCP port for P2P peer connections |
...
sudo systemctl restart ${{ values.componentId }}
```
